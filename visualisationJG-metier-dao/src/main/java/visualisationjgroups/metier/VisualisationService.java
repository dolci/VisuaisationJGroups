...
public class VisualisationService implements IVisualisationService{
		...
		public TreeMap<String, ArrayList> createGraph(ArrayList<Node> members) {
			...
			//color group
			members.get(0).setColor(Color.getColor().get(index));
			colorList.put(members.get(0).getMcast_addr()+":"+members.get(0).getMcast_port(), Color.getColor().get(index));
		    		 ...
		    		 for(Node n :listNode ){
		    			 ...
		    			 if( member != null){
		    				...
		    				if(!member.getCoordinateur().equals(n.getCoordinateur())){
		    					Changement change = new Changement();
		    					change.setDiscription("changeValueCoordinateurFrom "+member.getCoordinateur()+" "+n.getCoordinateur());change.setGraphe(graphe);
		    					addChangement(change);
		    					//update member
		...
		public void compareTwoList(Member member,Graphe graphe, ArrayList<String> viewList,ArrayList<String> viewListDb ,int db){
			...
			if(viewList.size() == 0 && viewListDb == null)
			 ...
			 if(viewList.size()>0 && viewListDb == null){
				 ...
				 if(db == 0){
					 change.setDiscription("newMemberView "+views);
					 member.setViewList(views);
				 }
				
				 else{
					 change.setDiscription("newMemberViewMaster "+views);
				 ...
				 if(db == 0){
					 member.setViewList("");
					 change.setDiscription("leftMemberView "+member.getViewList());
				 }
				 
				 else{
				    member.setViewMasterList("");
				    change.setDiscription("leftMemberViewMaster "+member.getViewList());
				 }
				 addChangement(change);
				 addMember(member);
			 }
			     ...
			     for(String view :viewList ){
				      if(!viewListDb.contains(view)){
    				      ...
    				      if(db == 0){
    				    	  ...
    				    	  change.setDiscription("newMemberView "+view);
    				      }
    				      else{
    				    	  member.setViewMasterList(member.getViewMasterList()+" "+view);
    				    	  change.setDiscription("newMemberViewMaster "+view);
    				      }
				...
				if(viewListDb != null){
				  for(String view0 :viewListDb ){
					   if(!viewList.contains(view0)){
						 Changement change = new Changement();
						 if (db == 0)
	    				    change.setDiscription("leftMemberView "+view0);
						 else
							 change.setDiscription("leftMemberViewMaster "+view0);
						 change.setGraphe(graphe);
	    				 addChangement(change);
		...
		public List<Member> getAllMemberDB(long idGraphe) {
			...
			return Lists.newArrayList(memberRepository.getAllMemberActif(idGraphe));
		}
		...
		public List<Member> getAllMemberActif(long idGrpahe) {
			...
			return Lists.newArrayList(memberRepository.getAllMemberActif(idGrpahe));
		}
