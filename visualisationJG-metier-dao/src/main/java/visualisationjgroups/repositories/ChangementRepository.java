...
//	@Query("select c from Changement c where c.graphe.id=?1 order by v.dateModif desc")
	...
	@Query("select c from Changement c where c.graphe.id=?1  and c.dateModif between ?2 and ?3 and c.heureModif between ?4 and ?5  order by c.dateModif,c.heureModif desc")
	Iterable<Changement> findByDateheureModifBetween (long idGraphe,Date datefrom,Date dateTo, String heureFrom,String heureTo);
}
